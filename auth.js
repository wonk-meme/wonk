...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Don't show auth modal automatically - user will click login button
  setupEventListeners();
});
...
async function loginWithPhantom() {
  try {
    ...
    if (!window.solana || !window.solana.isPhantom) {
      if (device.isMobile) {
        // Redirect to Phantom mobile app
        ...
        setTimeout(() => {
          ...
          } else if (device.userAgent.includes('iphone') || device.userAgent.includes('ipad')) {
            window.open('https://apps.apple.com/app/phantom-solana-wallet/id1598432977', '_blank');
          }
        }, 2000);
        
        showErrorMessage('Redirecting to Phantom app...');
        return;
      } else {
        // Desktop - redirect to extension installation
    ...
    if (window.adminPanel && window.adminPanel.isAdmin(currentUser.uid)) {
...
async function loadUserPoints() {
  try {
    if (window.wonkDB && currentUser) {
      ...
      userPoints = savedPoints ? parseInt(savedPoints) : 50;
...
async function voteForCrypto(cryptoName, cost) {
  ...
  try {
    ...
    showSuccessMessage('Processing vote on blockchain...');
    
    // Cast vote on blockchain for transparency
    let blockchainVote = null;
    if (window.blockchainVoting) {
      try {
        blockchainVote = await window.blockchainVoting.castVote(cryptoName, currentUser.uid);
        console.log('Blockchain vote successful:', blockchainVote);
      } catch (blockchainError) {
        console.warn('Blockchain vote failed, continuing with database vote:', blockchainError);
      }
    }
    ...
    if (window.wonkDB && currentUser) {
      ...
      await wonkDB.recordTransaction(currentUser.uid, 'vote', cost, {
        ...
        action: 'vote_cast',
        blockchainHash: blockchainVote ? blockchainVote.transactionHash : null,
        blockNumber: blockchainVote ? blockchainVote.blockNumber : null
      });
    }
    ...
    addVoteToFeed(currentUser.uid, cryptoName, blockchainVote);
    
    if (blockchainVote) {
      showSuccessMessage(`Vote cast successfully on blockchain! TX: ${blockchainVote.transactionHash.substring(0, 10)}...`);
    } else {
...
function addVoteToFeed(userId, cryptoName = null, blockchainVote = null) {
  const liveVotesFeed = document.getElementById('liveVotesFeed');
  if (!liveVotesFeed) return;
  ...
  const blockchainInfo = blockchainVote ? 
    `<div class="text-xs text-green-400 mt-1">
      <i class="fas fa-link mr-1"></i>
      Blockchain: ${blockchainVote.transactionHash.substring(0, 10)}...
    </div>` : '';
